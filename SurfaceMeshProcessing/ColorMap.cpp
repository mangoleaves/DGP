...
OpenMesh::Vec3d ColorMap::MapToColor(double value)
{
	...
	if (mapValue < 0.25)
	{
		return OpenMesh::Vec3d(0, mapValue * 4, 1);
	}
	else if (mapValue < 0.5)
	{
		return OpenMesh::Vec3d(0, 1, 1 - 4 * (mapValue - 0.25));
	}
	else if (mapValue < 0.75)
	{
		return OpenMesh::Vec3d(4 * (mapValue - 0.5), 1, 0);
	}
	else
	{
		return OpenMesh::Vec3d(1, 1 - 4 * (mapValue - 0.25), 0);
	}
}
